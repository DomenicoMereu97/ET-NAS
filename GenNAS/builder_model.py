from GenNAS.fbnas import NDS
from copy import deepcopy
import torch.nn as nn
import torch.nn.functional as F
from GenNAS.model_wrapper import *
from GenNAS.pycls.models.nas.genotypes import GENOTYPES, Genotype
import numpy as np
...
class ModelBuilder():
    ...
    def get_model(self, arch_config):
        if self.search_space in ['DARTS','DARTS_in','DARTS_fix-w-d','DARTS_fix-w-d_in',\
            ...
            model = NB101Wrapper(arch_config, self.init_channels, self.last_channels, self.output_size, self.num_lables)
        elif self.search_space == 'nasbench201':
            model = NB201Wrapper(arch_config, self.init_channels, self.last_channels, self.output_size, self.num_labels, self.dataset)
