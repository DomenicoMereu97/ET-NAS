from GenNAS.foresight.dataset import *
from GenNAS.utils.tricks import *
from GenNAS.configs import *
...
class CVTask():
    def __init__(self, args):#currently we support C10/C100/IN16
        ...
        train_loader, val_loader = get_cifar_dataloaders(int(self.config['batchsize']), int(self.config['batchsize']), self.dataset, self.workers,if_split = self.if_split, pad=False, meanstd = self.meanstd, datadir = self.data_loc )
    ...
    def _build_target(self):
        ...
        for layer in range(3):
            ...
            if config['feature'][str(layer)]['sin1d'] is not None:
                target += generate_wave(config['feature'][str(layer)]['sin1d']['range'],config['batchsize'],int(config['last_channel_l%d'%layer]*self.last_channels), if_2d = False, local=config['feature'][str(layer)]['sin1d']['local'], input_size = self.input_size, output_size = self.output_size) * config['feature'][str(layer)]['sin1d']['level']
            if config['feature'][str(layer)]['sin2d'] is not None:
                target += generate_wave(config['feature'][str(layer)]['sin2d']['range'],config['batchsize'],int(config['last_channel_l%d'%layer]*self.last_channels), if_2d = True, local=config['feature'][str(layer)]['sin2d']['local'], input_size = self.input_size, output_size = self.output_size) * config['feature'][str(layer)]['sin2d']['level']
            if config['feature'][str(layer)]['resize'] is not None:
                resizedata = F.interpolate(data,(self.output_size,self.output_size))
            ...
            if config['feature'][str(layer)]['dot'] is not None:
                target += config['feature'][str(layer)]['dot']['level'] * generate_gaussian_dot(config['batchsize'],c = int(config['last_channel_l%d'%layer]*self.last_channels),n = int(config['feature'][str(layer)]['dot']['partial']* self.output_size * self.output_size),local = config['feature'][str(layer)]['dot']['local'], gaussian = False, input_size = self.input_size, output_size = self.output_size)

            if config['feature'][str(layer)]['dot_gaussian'] is not None:
                target += config['feature'][str(layer)]['dot_gaussian']['level'] * generate_gaussian_dot(config['batchsize'],c = int(config['last_channel_l%d'%layer]*self.last_channels),n = int(config['feature'][str(layer)]['dot_gaussian']['partial']* self.output_size * self.output_size),local = config['feature'][str(layer)]['dot_gaussian']['local'], gaussian = True, input_size = self.input_size, output_size = self.output_size)
